 ...
 * order. REAL files are placed under `${PROJ}/wip/bundles/a/installer-scripts/` (NOT under payload/) so the on-disk
...
function seed(
  ...
  for (const rel of opts.placed ?? []) {
    ...
    fs.write(`${PROJ}/wip/bundles/a/installer-scripts/${rel}`, `#!/bin/sh\n# ${rel}\n`);
  }
  ...
  for (const rel of opts.placedTemplates ?? []) {
    fs.write(`${PROJ}/wip/bundles/a/payload/templates/${rel}`, `template ${rel}\n`);
  }
...
describe("bundle <id> scripts add (task-71)", () => {
  it("AC#1 — registers an existing path; NO file content written; comment + key order preserved; exit 0", async () => {
    ...
    const fileBefore = fs.read(`${PROJ}/wip/bundles/a/installer-scripts/probe.sh`);
    const i = io();
    expect(
    ...
    expect(fs.read(`${PROJ}/wip/bundles/a/installer-scripts/probe.sh`)).toBe(fileBefore);
  ...
  it("AC#2 — a file placed under payload/installer-scripts (wrong dir) does NOT satisfy the check (sibling-of-payload)", async () => {
    ...
    fs.write(`${PROJ}/wip/bundles/a/payload/installer-scripts/probe.sh`, "#!/bin/sh\n");
    const before = fs.read(`${PROJ}/wip/bundles/a/bundle.yml`);
    const i = io();
    expect(
      await run(["bundle", "a", "scripts", "add", "probe.sh", "-C", PROJ], deps(fs, backlog), i),
...
describe("bundle <id> scripts remove (task-73)", () => {
  ...
  it("AC#2 — the file content is LEFT on disk (deregister, not delete)", async () => {
    const { fs, backlog } = seed({ placed: ["probe.sh", "sub/smoke.sh"], aYml: A_YML_WITH_REFS });
    const contentBefore = fs.read(`${PROJ}/wip/bundles/a/installer-scripts/probe.sh`);
    expect(
      await run(
    ...
    expect(fs.exists(`${PROJ}/wip/bundles/a/installer-scripts/probe.sh`)).toBe(true);
    expect(fs.read(`${PROJ}/wip/bundles/a/installer-scripts/probe.sh`)).toBe(contentBefore);
  });
