...
- **Skill Story Definition-of-Done supplement:** for any story that creates or changes a WPM-owned skill, the implementer refreshes to the latest official stable authoring helper available at execution time and actually invokes, during that story, either Codex's official `$skill-creator` or Claude Code's official `skill-creator@claude-plugins-official`. The story evidence records the authoring helper and host version, the then-current official Codex and Claude Code skill-authoring source links and access date, deterministic native-path, frontmatter, discovery and explicit-invocation identity, trigger/non-trigger contract, and source-free portability evidence for both platforms. A prior skill story's authoring or evidence does not satisfy this story-level gate.
- **Consolidated live supported-client gate:** fresh live Codex and externally authenticated live Claude Code discovery, explicit invocation, natural-language trigger, non-trigger, and representative-outcome parity for all six WPM-owned skills run once after all scoped implementation against the exact final packed revision. This gate runs before final handoff or activation; host authentication is an external prerequisite and is not a WPM capability. Individual skill stories may retain truthful host diagnostics but neither claim nor block on final live-host behavioral parity.
- The Skill Story Definition-of-Done supplement is one conditional shared backlog DoD item, not repeated acceptance-criteria text. Each skill story owns packed-install availability and generated-deliverable non-leakage evidence for its own artifact. Exact archive/source-free evidence is accepted once against the stable product/test hash; review may adopt an earlier run only when no product/test byte changed and the evidence binds that exact hash.
...
- Stories 1.2 and 1.3 establish generic, revision-scoped ship-set inspection and packed-install harnesses for whatever assets the evaluated source revision declares. Each later WPM-owned-skill story owns adding and proving its own packaged asset; Story 2.11 proves the complete six-skill family. After all scoped stories, the final cold gate runs the exact full local suite once, reruns those harnesses from the final revision, regenerates the inactive candidate from the resulting exact bytes, assesses only that refreshed candidate, and runs the consolidated live Codex and authenticated live Claude parity matrix for all six exact packaged skills before final handoff or activation.
...
After Story 3.3, the final cold gate checks out the exact final revision in a clean environment, runs dependency installation, typecheck, repository lint, build, and the exact full test suite once, reruns the Story 1.2 package inspection and Story 1.3 packed-install harness, regenerates and rebinds the Story 1.4 inactive candidate from those exact bytes, reruns Stories 1.5–1.7 assessments against only that refreshed candidate, and runs fresh live Codex plus externally authenticated live Claude discovery, explicit invocation, natural-language trigger, non-trigger, and representative-outcome parity for all six exact packaged WPM skills. This is verification of current state, not another story or a remote release action; it must pass before final handoff or activation.
