...
- **Consolidated final verification gate:** after all scoped implementation, one clean exact revision runs dependency installation, typecheck, repository lint, build, and the exact full test suite; regenerates and rebinds exact package, packed-install, candidate, and channel evidence; and runs fresh live Codex plus externally authenticated live Claude Code discovery, explicit-invocation, natural-language trigger, non-trigger, and representative-outcome parity for all six exact packaged WPM-owned skills. This gate runs before final handoff or activation; host authentication is an external prerequisite and is not a WPM capability. Individual skill stories retain deterministic two-platform contract, exact-package, and non-leakage evidence but do not claim or block on final live-host behavioral parity.
...
- Stories 1.2 and 1.3 establish generic, revision-scoped ship-set inspection and packed-install harnesses for whatever assets the evaluated source revision declares. Each later WPM-owned-skill story owns adding and proving its own packaged asset; Story 2.11 proves the complete six-skill family. After all scoped stories, the final cold gate runs the exact full suite once, reruns those harnesses from the final revision, regenerates the inactive candidate from the resulting exact bytes, assesses only that refreshed candidate, and runs consolidated fresh live Codex plus externally authenticated live Claude parity for all six exact packaged skills before final handoff or activation.
...
After Story 3.3, the final cold gate runs dependency installation, typecheck, repository lint, build, and the exact full test suite once; reruns the Story 1.2 package inspection and Story 1.3 packed-install harness against that final revision; regenerates and rebinds the Story 1.4 inactive candidate from those exact bytes; reruns Stories 1.5–1.7 assessments against only that refreshed candidate; and runs fresh live Codex plus externally authenticated live Claude discovery, explicit invocation, natural-language trigger, non-trigger, and representative-outcome parity for all six exact packaged WPM skills. This is verification of current state, not another story or a remote release action; it must pass before final handoff or activation.
