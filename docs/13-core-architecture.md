...
`12` chose the stack and drew the directory scaffold. This doc architects the **core app** that lives inside it: the module-level contracts, the data flow, and the seams that keep the thing testable as it grows from the foundational backlog into every command leaf. It is the design that the foundational backlog implements tier by tier — the model and ports (tasks 10–15), the services (16–22), the operations and cross-cutting machinery (23–26), and the driving CLI adapter (27–29) — proven end-to-end by the walking skeleton (task-33).
...
The foundational tasks build this architecture bottom-up, in FOUNDATION.md's phase order: the **model** is task-10 and the three **schemas** task-11; the **ports** are task-12 (`FileSystem`, real + in-memory), task-14 (`BacklogMd`, shell-out + fake), and task-15 (`Clock` + `Environment`), with task-13 the comment-preserving YAML behind the fs adapter. The **services** are task-16 (`render`), task-17 (two-tier `template-resolver`), task-18 (`version-constraint`), task-19 (`derived-artefacts`, including `scope-plan`), task-20 (`validate`), task-21 (`materialisation`), and task-22 (`integrity`). The **operations tier and its cross-cutting machinery** are task-23 (the typed error model + exit-code mapping), task-24 (context resolution), task-25 (the shared mutation lifecycle harness), and task-26 (one representative operation end-to-end). The **driving adapter** is task-27 (the commander composition root + registration pattern + DI + error handler), with task-28 (the `--help` content contract) and task-29 (tab-completion) layered on. The **walking skeleton** (task-33) is the first end-to-end thread — `commands → operation → services → ports → fs` against a real tmpdir — proving the hexagon composes before the per-command leaves are filled in.
